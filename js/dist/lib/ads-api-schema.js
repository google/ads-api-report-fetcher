...
export class AdsApiSchemaRest {
    ...
    async getResource(name) {
        ...
        if (!restSchemaObject) {
            this.logger.error(`[AdsApiSchemaRest.getResource] restSchemaObject is null for name: ${name}. Available keys in loadedSchema.schemas:`, Object.keys(loadedSchema.schemas));
            throw new Error(`Schema for '${name}' (checked ${schemaId}) not found in REST API schema.`);
        }
        else {
            this.logger.debug(`[AdsApiSchemaRest.getResource] Found restSchemaObject for ${schemaId}. Properties:`, Object.keys(restSchemaObject.properties || {}));
        }
        const protoFields = {};
        ...
        const transformedResource = {
            name: (restSchemaObject.id || schemaId).split('__').pop(),
            fields: protoFields,
            nested: {},
        };
    ...
    transformRestPropertyToProtoFieldMeta(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    propDetails) {
        ...
        if (propDetails.type === 'array') {
            ...
            if (propDetails.items) {
                if (propDetails.items.$ref) {
                    ...
                    if (propDetails.items.format) {
                        fieldType = this.mapRestFormatToType(propDetails.items.type, propDetails.items.format);
                    }
                }
            }
        }
            ...
            fieldType = this.mapRestFormatToType(propDetails.type, propDetails.format);
        }
        return {
            type: fieldType,
            rule: rule,
    ...
    mapRestFormatToType(type, format) {
        if (format) {
            switch (format) {
                case 'int64':
                    return 'int64';
        ...
        if (type === 'boolean')
            return 'bool';
        if (type === 'integer')
            return 'int64'; // Default to 64 bit unless formatted
        if (type === 'number')
            return 'double';
        return type;
    }
    async getFieldType(parentType, nameParts) {
        if (!nameParts || nameParts.length === 0) {
            throw new Error('ArgumentException: nameParts is empty');
        }
        if (!parentType || !parentType.name) {
            throw new Error('ArgumentException: parentType or parentType.name was not specified');
        }
        const loadedSchema = await this.getLoadedSchema();
        if (!loadedSchema || !loadedSchema.schemas) {
            throw new Error('REST API schema has not been loaded or is invalid.');
        ...
        if (!currentRestSchema) {
            ...
            if (rowSchema && rowSchema.properties) {
                const camelName = parentType.name.charAt(0).toLowerCase() + parentType.name.slice(1);
        ...
        if (!currentRestSchema) {
            throw new Error(`Could not find schema for parent type ID '${parentType.name}'`);
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        let finalPropDetails;
        ...
        for (let i = 0; i < nameParts.length; i++) {
            ...
            if (finalPropDetails.type === 'array') {
                ...
                if (finalPropDetails.items && finalPropDetails.items.$ref) {
                    currentRestSchema =
                        loadedSchema.schemas[finalPropDetails.items.$ref.replace(/^#\/schemas\//, '')];
                    if (!currentRestSchema) {
                        throw new Error(`Could not resolve $ref '${finalPropDetails.items.$ref}' for array item`);
                    }
                }
                else if (finalPropDetails.items && finalPropDetails.items.type) {
                    if (i < nameParts.length - 1) {
                        throw new Error(`Cannot access further properties on an array of non-$ref items: ${snakeCasePart}`);
                    }
                }
                else {
                ...
                currentRestSchema =
                    loadedSchema.schemas[finalPropDetails.$ref.replace(/^#\/schemas\//, '')];
                if (!currentRestSchema) {
                ...
                if (i < nameParts.length - 1) {
                    if (finalPropDetails.type === 'object' &&
                        finalPropDetails.properties) {
                        currentRestSchema = finalPropDetails;
                    }
                    else {
                        throw new Error(`Cannot access further properties on a non-object/non-$ref type: ${snakeCasePart}`);
                    }
                }
                isRepeatedContext = false;
            }
        ...
        if (!finalPropDetails) {
            throw new Error(`Could not resolve field path '${nameParts.join('.')}' in type '${parentType.name}'`);
        }
        let fieldTypeKind;
        let determinedTypeName;
        let typeValue;
        if (finalPropDetails.enum) {
            fieldTypeKind = FieldTypeKind.enum;
            const currentFieldName = snakeToPascalCase(nameParts[nameParts.length - 1]);
            const owningTypeName = (currentRestSchema.id || parentType.name)
                .split('__')
                .pop();
            determinedTypeName = `${owningTypeName}${currentFieldName}`;
            const enumValues = {};
            finalPropDetails.enum.forEach((val, idx) => {
                enumValues[val] = idx;
            });
            ...
            if (!referencedSchemaObject) {
                throw new Error(`Could not find schema for $ref ID: ${schemaIdToLookup}`);
            }
            const protoFields = {};
            if (referencedSchemaObject.properties) {
                for (const propName in referencedSchemaObject.properties) {
                    protoFields[propName] = this.transformRestPropertyToProtoFieldMeta(referencedSchemaObject.properties[propName]);
                }
            }
            typeValue = {
        ...
        else if (finalPropDetails.type === 'object' &&
            finalPropDetails.properties) {
            fieldTypeKind = FieldTypeKind.struct;
            determinedTypeName =
                finalPropDetails.id || `${nameParts.join('_')}Struct`;
            const inlineStructFields = {};
            for (const propName in finalPropDetails.properties) {
                inlineStructFields[propName] =
                    this.transformRestPropertyToProtoFieldMeta(finalPropDetails.properties[propName]);
            }
            ...
            if (finalPropDetails.items.type && !finalPropDetails.items.$ref) {
                ...
                determinedTypeName = this.mapRestFormatToType(finalPropDetails.items.type, finalPropDetails.items.format);
                typeValue = determinedTypeName;
            }
            else if (finalPropDetails.items.$ref) {
                fieldTypeKind = FieldTypeKind.struct;
                const fullSchemaId = finalPropDetails.items.$ref.replace(/^#\/schemas\//, '');
                const schemaIdToLookup = fullSchemaId;
                const referencedSchemaObject = loadedSchema.schemas[schemaIdToLookup];
                if (!referencedSchemaObject) {
                    throw new Error(`Could not find schema for array item $ref ID: ${schemaIdToLookup}`);
                }
                const protoFields = {};
                if (referencedSchemaObject.properties) {
                    for (const propName in referencedSchemaObject.properties) {
            ...
            fieldTypeKind = FieldTypeKind.primitive;
            determinedTypeName = this.mapRestFormatToType(finalPropDetails.type, finalPropDetails.format);
            typeValue = determinedTypeName;
        }
        return {
    ...
    async getTypePrimitiveFields(type) {
        const loadedSchema = await this.getLoadedSchema();
        ...
        if (!restSchemaObject) {
            ...
            const rowSchema = loadedSchema.schemas[rowSchemaId];
            if (rowSchema && rowSchema.properties) {
                const camelName = type.name.charAt(0).toLowerCase() + type.name.slice(1);
                const propDetails = rowSchema.properties[camelName];
                if (propDetails && propDetails.$ref) {
                    restSchemaObject =
                        loadedSchema.schemas[propDetails.$ref.replace(/^#\/schemas\//, '')];
        ...
        if (!restSchemaObject || !restSchemaObject.properties) {
            this.logger.warn(`Schema object for type name '${type.name}' not found or has no properties.`);
            return [];
        }
        const primitiveFields = [];
        for (const propName in restSchemaObject.properties) {
            const propDetails = restSchemaObject.properties[propName];
            const isPrimitive = propDetails.type === 'string' ||
            ...
            if (propDetails.type !== 'array' &&
                ...
                if (type.fields && type.fields[propName]) {
                    primitiveFields.push(Object.assign({}, type.fields[propName], { name }));
                }
                else {
                    primitiveFields.push(Object.assign(this.transformRestPropertyToProtoFieldMeta(propDetails), { name }));
                }
            }
        }
