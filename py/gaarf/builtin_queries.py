...
def get_ocid_mapping(report_fetcher: gaarf.report_fetcher.AdsReportFetcher,
    ...
    """
    query = ('SELECT customer.id AS account_id, '
...
def get_account_hierarchy_flattened(
    ...
    seed_accounts = accounts if isinstance(accounts,
                                           abc.MutableSequence) else [accounts]
        ...
        for i in range(1, max(df.level) + 1):
            if i == 1:
                ...
                mcc_ids = n_mcc_df.merge(
                    ...
                    left_on='mcc_id',
                    right_on='account_id')[[
                        'root_mcc_id', 'root_mcc_name', 'account_id_x',
