...
class AdsRowParser {
    parseRow(row, query) {
            ...
            if (value && customizer) {
                if (lodash_1.default.isArray(value)) {
                    ...
                    for (let j = 0; j < value.length; j++) {
                        new_value[j] = this.getValueWithCustomizer(value[j], customizer);
                    }
                    value = new_value;
                }
                else {
                    value = this.getValueWithCustomizer(value, customizer);
                }
            }
        ...
        for (let i = 0; i < query.fields.length; i++) {
            ...
            if (colType.kind === _1.FieldTypeKind.enum && colType.repeated && lodash_1.default.isArray(value)) {
                for (let j = 0; j < value.length; j++) {
                    let subval = value[j];
            ...
            else if (colType.kind === _1.FieldTypeKind.enum) {
            ...
            else if (colType.kind === _1.FieldTypeKind.struct) {
    ...
    getValueWithCustomizer(value, customizer) {
        if (!value)
            return value;
        if (customizer.type === types_1.CustomizerType.NestedField) {
            value = (0, utils_1.navigateObject)(value, customizer.value);
        }
